...
router.post("/signup", async (req, res) => {
    const { name, phone, email, password, userType, course_id } = req.body;
    try {
        ...
        con.query(sql, [email], async (err, result) => {
            ...
            if (result.length > 0) {
                ...
                if (userType == "alumnus") {
                    ...
                    const alumnusSql = "INSERT INTO alumnus_bio(name,phone, email, course_id) VALUES(?,?,?,?)";
                    con.query(alumnusSql, [name,phone, email, course_id], async (alumnusErr, alumnusResult) => {
                        if (alumnusErr) {
                        ...
                        const userSql = "INSERT INTO users(name,phone, email, password, type, alumnus_id) VALUES(?,?,?,?,?,?)";
                        con.query(userSql, [name, phone, email, hashedPassword, userType, alumnusId], (userErr, userResult) => {
                            if (userErr) {
                    ...
                    const sql = "INSERT INTO users(name, phone, email, password, type) VALUES(?,?,?,?,?)";
                    con.query(sql, [name,phone, email, hashedPassword, userType], (err, result) => {
                        if (err) {
